...
packages. Additionally, we'll use data from the ipumsexamples package.
If any of these packages are uninstalled, you can do so with:
...
```{r, include=FALSE}
installed_ipumsexamples <- requireNamespace("ipumsexamples")
...
if (!installed_ipumsexamples) {
  message(
    "ipumsexamples is not installed and example data cannot be loaded.",
    "Code in this example will not be evaluated."
  )
...
If using the API, use `remove_from_extract()` to easily remove unneeded
extract values:

```{r, eval=TRUE}
sample_cps_extract <- define_extract_cps(
  description = "Sample extract for big data vignette",
  samples = "cps2011_03s",
  variables = c("PERNUM", "FOODSTMP", "AGE", "EMPSTAT", "AHRSWORKT", "HEALTH")
)

new_cps_extract <- remove_from_extract(
  sample_cps_extract,
  variables = c("EMPSTAT", "HEALTH")
)

new_cps_extract
```

To obtain the updated data, resubmit the extract request and download
the new files:

```{r, eval=FALSE}
data <- submit_extract(new_cps_extract) %>%
  download_extract(wait = TRUE) %>%
  read_ipums_micro()
```

Use `vignette("ipums-api", package = "ipumsr")` to get more information
about using the IPUMS API to make extract requests.

### Select cases {#select-cases}

For microdata projects, another good option for reducing extract size is
to use the "Select Cases" option before submitting. This allows you to
produce an extract containing data for a particular subset of values in
an included variable. For example, you could select cases for `AGE` such
that your extract only includes those older than 65.

In general, the "Select Cases" option is available on the last page
before submitting an extract request.

Case selection is not yet available via the IPUMS API, but is slated to
be supported in the future.

### Use a sampled subset of the data

Yet another option (also only for microdata projects) is to take a
random subsample of the data before producing your extract. Use the
"Customize Sample Size" option to do so. This also appears on the final
page before submitting an extract request.
...
First, we'll load our example data:

```{r}
cps_ddi_file <- ipumsexamples::ipums_extra_example("cps_00011.xml")
cps_data_file <- ipumsexamples::ipums_extra_example("cps_00011.dat.gz")
```
...
variable (see `vignette("value-labels", package = "ipumsr")`), and count
the people in each group.

```{r}
read_ipums_micro(cps_ddi_file, data_file = cps_data_file, verbose = FALSE) %>%
  mutate(
...
```{r}
data <- read_ipums_micro(cps_ddi_file, data_file = cps_data_file, verbose = FALSE) %>%
  mutate(
    HEALTH = as_factor(HEALTH),
...
#### Chunked case selection

Sometimes you may only be interested in a subset of the data, but can't
read the entire dataset into memory to remove the unused cases.

It may make the most sense to do this when creating your extract, which
can be done through the IPUMS extract interface (see the [select
cases](#select-cases) section above). R is also likely to be slower for
this functionality than other options, like Unix commands `awk` and
`sed`.

However, ipumsr `chunked` functions can emulate this functionality by
filtering the chunked data in the callback function:
...
This option may be particularly useful if you want to subset based on
complex logic that would be hard to code into the IPUMS extract system
or Unix tools.
...
`read_ipums_micro_yield()` and `read_ipums_micro_yield_list()` grant you
...
we'll use RSQLite to load the data into an in-memory databse. (We use
