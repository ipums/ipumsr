...
comma-delimited files). Metadata files generally contain information
about the data file and its contents, including variable descriptions
and parsing instructions for fixed-width data files. Microdata projects
provide metadata in DDI (.xml) files. Aggregate data projects provide
metadata in either .txt or .csv formats.
...
of the project website. This leads to that project's extract interface,
which allows you to specify the data you'd like to download. The data
selection parameters will differ across projects; see each project's
...
download the data file. Downloading the DDI file may differ based on
your browser. Typically, you can right-click the file and select "Save
as..." (on Safari, select "Download Linked File"). This should save the
file with the required .xml extension. (Make sure the file is *not*
saved with an .html extension.)

TODO: Update this image as the CSS for the website has changed.

![Annotated screenshot for downloading microdata (may look different on
your
computer)](microdata_annotated_screenshot.png){alt="Annotated screenshot for downloading microdata (may look different on your computer)"
width="700px"}

#### For aggregate data projects

Simply download an extract's data by clicking the green "Tables" button
(for tabular data) and/or "GIS Files" button (for spatial boundary data)
in the "Download Data" column.

### Downloading via the IPUMS extract API

Extract requests can also be created and submitted from within R by
interacting with the IPUMS extract API. The IPUMS extract API currently
supports the following collections:
...
directly to your local machine without ever having to leave R:
...
data_files <- download_extract(submitted_extract, wait = TRUE)
```
...
```{r, eval=FALSE}
past_extracts <- get_extract_info("nhgis")
```

See the associated [vignette](ipums-api.html) for more details about how
to use ipumsr to interact with the IPUMS extract API.
...
Users can also get contextual details for specific variables:
...
See the [value labels](value_labels.html) vignette for more details.
