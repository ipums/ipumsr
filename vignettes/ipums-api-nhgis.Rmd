...
specific set of IPUMS NHGIS data, use `define_extract_agg_data()`.
...
```{r}
nhgis_ext <- define_extract_agg_data(
  "nhgis",
...
`ds_spec` or `tst_spec` objects in `define_extract_agg_data()`:
...
See the documentation for `define_extract_agg_data()` for more details on
these options.
