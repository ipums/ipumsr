...
bundle and allows for aggregating microdata and raster data into the boundaries. For more about using the geographic data in IPUMS Terra, see the Terra vignette (using command `vignette("ipums-terra", package = "ipumsr")`)
...
## Basic workflow for microdata projects (IPUMS DHS, IPUMS International, and IPUMS USA)
...
gives a message when some of the data has been dropped by the join (though in
this example there are no mismatches - see the [complicated example](#complicated) 
below for an example where this does happen). 
...
```{r}
nhgis_ddi <- read_ipums_codebook(ex_file("nhgis0706_csv.zip")) 
nhgis <- read_nhgis_sf(
  data_file = ex_file("nhgis0706_csv.zip"),
  shape_file = ex_file("nhgis0706_shape_small.zip"),
  verbose = FALSE
)
```
