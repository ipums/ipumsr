...
The IPUMS API supports two types of functionality.

-   The **extract** component of the API allows registered IPUMS users
    to define extract requests, submit them for processing, and download
    completed extract files, all without leaving R.

-   The **metadata** component of the API allows users to discover and
    explore data sources as well as retrieve codes that can be used when
    forming extract requests to submit to the API for processing.

ipumsr provides functions that support both of these components.
...
functionality may not yet be stable. Additionally, the IPUMS API for
microdata collections is still in beta testing. Users will need to
...
directly to these functions, but storing it in an environment variable saves you 
some typing and helps prevent you
from inadvertently sharing your key with others (for instance, on
GitHub).

You can save your API key to the `IPUMS_API_KEY` environment variable with
...
The rest of this vignette assumes you have obtained an API key and stored it in 
...
request's status. This extract hasn't yet been submitted,
...
For microdata variable names, refer to the relevant collection's online extract 
builder:
...
create an identical extract request themselves. The terms of use for most IPUMS 
collections prohibit the redistribution of IPUMS data, but don't prohibit 
sharing data extract definitions.

ipumsr facilitates this type of sharing with `save_extract_as_json()` and
