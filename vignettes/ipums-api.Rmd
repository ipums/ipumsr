...
IPUMS provides two closely-related APIs: an extract API and a metadata
API. ipumsr provides functions that make it easier to interact with both
of these APIs.

-   The **IPUMS extract API** allows registered IPUMS users to define
    extract requests, submit them for processing, and download completed
    extract files, all without leaving R.

-   The **IPUMS metadata API** allows users to discover and explore data
    sources as well as retrieve codes that can be used when forming
    extract requests that will be submitted to the IPUMS extract API.

Use of these APIs enables the adoption of a programmatic workflow that
can help users to:
...
The basic workflow for interacting with the IPUMS extract API is as
follows:
...
The **IPUMS extract API** is currently available for the following
collections:
...
The **IPUMS metadata API** is currently available for the following
collections:
...
The API tools in ipumsr are in active development, and some
functionality may not yet be stable. Additionally, the IPUMS extract API
for microdata collections is still in beta testing. Users will need to
request beta access for these collections by emailing
[ipums+api\@umn.edu](mailto:ipums+api@umn.edu){.email}.

## Set up your API key

To interact with the IPUMS APIs, you'll need to register for access with
...
key](https://account.ipums.org/api_keys). (For IPUMS USA and CPS, you'll
also need to request API beta access by emailing
[ipums+api\@umn.edu](mailto:ipums+api@umn.edu){.email} as mentioned
above.)
...
directly to these functions, but we recommend storing your key in your
environment. This makes writing code less tedious and helps prevent you
from inadvertently sharing your key with others (for instance, on
GitHub).

You can save your API key to your environment with
`set_ipums_api_key()`. To save your key for use in future sessions, set
`save = TRUE`. This will add your API key to your `.Renviron` file in
...
The rest of this vignette assumes you have obtained an API key and have
it stored in your environment.
...
the `ipums_extract` class, which can be handled by other extract API
functions (see `?ipums_extract`). Furthermore, these objects will have a
subclass for the particular collection with which they are associated.
...
request's status. As expected, this extract hasn't yet been submitted,
and therefore has no extract request number:
...
used for the data sources you want to include.
...
used to determine' the name used by the extract API to reference a data
source of interest.
...
One exciting feature enabled by the IPUMS extract API is the ability to
share a standardized extract definition with other IPUMS users so that
they can create an identical extract request themselves (recall that
IPUMS data cannot be redistributed themselves, but the definition to
reproduce them can).

ipumsr facilitates this with `save_extract_as_json()` and
`define_extract_from_json()`, which read and write `ipums_extract`
...
load and submit to the extract API themselves.
...
tables, pass a `list` to the argument instead of a vector. This allows
you to indicate which parent field should have the subfield modified.
This is analogous to the syntax that can be used when defining an NHGIS
extract request.
...
can't remember the exact number, you can use `get_extract_info()` to
peruse your recent extract requests. Just provide a collection to the
`extract` argument instead of a collection with an extract number.
...
```{r}
nhgis_extracts <- get_extract_info("nhgis", how_many = 3)
...
```{r}
usa_extract_tbl <- get_extract_info("usa", how_many = 5, table = TRUE)
...
```{r}
identical(
  extract_list_to_tbl(get_extract_info("usa")),
  get_extract_info("usa", table = TRUE)
)
```
