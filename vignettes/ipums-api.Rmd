...
download. Otherwise, the links will be empty.
...
`"collection:number"` or as a length-2 vector: `c(collection, number)`. Several 
other API functions also support this syntax as well.

```{r, eval=FALSE}
usa_ext_10 <- get_extract_info("usa:10")

# Or, alternatively:
get_extract_info(c("usa", 10))
```
...
usa_ext_submitted <- wait_for_extract(usa_ext_submitted)
...
For microdata collections, this is the path to the DDI
codebook (.xml) file, which can be used to read the associated data
(contained in a .dat.gz file). For NHGIS, this will be a path to the .zip 
...
busy. If you like, you can set a maximum wait time and adjust other API polling
parameters in `wait_for_extract()`.

```{r, eval=FALSE}
downloadable_extract <- wait_for_extract(usa_ext_submitted)
...
See the [associated vignette](ipums-read.html) for more information about loading
IPUMS data into R.
...
In this case, the best approach is to create a new extract definition
(using a `define_extract_*()` function) that reproduces the specifications of 
that extract along with the desired modifications. While this
...
them with the familiar API functions.

```{r}
usa_extracts[[2]]$status
```
...
Or we can use the `purrr::map()` family to browse:
...
These functions are compatible with one another
