...
Define an extract for an IPUMS microdata collections with
...
case selections, attached characteristics, and data quality flags.
...
Acceptable values are `"spouse"`, `"mother"`, `"father"`, `"head"`, `"mother2"`,
and `"father2"`.
...
For pre-defined time use variables, simply provide the name:
...
define_extract_micro(
  ...
  samples = "at2017",
  time_use_variables = "ACT-PCARE"
)
```

You can also request time use variables that you have defined yourself. In this
case, use the `tu_var_spec()` helper to provide the time use variable name
and your user email to identify the account the variable was created under:
