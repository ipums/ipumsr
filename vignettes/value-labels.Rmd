...
IPUMS data comes with 3 primary types of variable-level metadata:
...
-   ***Value labels*** link particular data values to more meaningful
    ...
    and "Very good" health. This mapping would be contained in the
    variable's value labels.
...
Compared to factors, `labelled` vectors have a few properties that make
them more suitable for working with IPUMS data:

-   They provide more complete contextual information about the meaning
    of certain values
...
### Drawbacks of `labelled` variables

Note that *both* of the means calculated above are suspect.
...
`lbl_na_if()`. `lbl_na_if()` uses a function of the value labels to
handle the conversion. The function should take each value label as its
input and output `TRUE` for those observations whose values should be
converted to `NA`.
...
-   The `.val` argument references the *values* in the value labels

-   The `.lbl` argument references the *labels* in the value labels

For instance, to convert all values equal to `99999999` to `NA`, we can
...
inctot_na <- lbl_na_if(
  cps$INCTOT, 
  function(.val, .lbl) .val == 99999999
)

# All 99999999 values have been converted to NA
any(inctot_na == 99999999, na.rm = TRUE)
...
```{r, eval=FALSE}
lbl_na_if(cps$INCTOT, ~ .val == 99999999)
```

or for R versions greater than 4.1, base R's anonymous function syntax:

```{r, eval=FALSE}
lbl_na_if(cps$INCTOT, \(.val, .lbl) .val == 99999999)
```
...
`lbl_relabel()` can be used to create new value labels, often to
...
-   On the right-hand side, provide a function that returns `TRUE` for
    those values that should be relabelled with the new label from the
    left-hand side
...
`lbl_collapse()` uses a function that takes `.val` and/or `.lbl`
[arguments](#syntax) and returns the new value each input value should
be assigned to. The label of the lowest initial value is used for each
