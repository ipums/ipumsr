...
This structure contains a single row for each record, regardless of
record type. Variables that do not apply to a particular record type
will be filled with `NA` in rows of that record type.

To read data in list format, use `read_ipums_micro_list()`. This splits
the record types into a list where each element contains all the records
for a given type:
...
file to load. `file_select` accepts a tidyselect selection helper when
...
While generally less useful, `file_select` also accepts a precise string
of the file name to load or an index position:
...
-   For microdata projects, spatial data are distributed in shapefile
    format on dedicated geography pages separate from the standard
...
archives and the `file_select` syntax for file selection.
...
These data can then be joined on associated tabular data. To preserve
