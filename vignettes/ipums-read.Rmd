...
# Example data
cps_ddi_file <- ipums_example("cps_00006.xml")
...
Note that you provide the path to the DDI file, *not* the data file. This is
because ipumsr needs to find both the DDI and data files to read in your data,
and the DDI file includes the name of the data file, whereas the data file 
...
```{r}
cps_hier_ddi <- read_ipums_ddi(ipums_example("cps_00010.xml"))

read_ipums_micro(cps_hier_ddi)
```

The long format consists of a single `data.frame` that includes rows with 
varying record types. In this example, some rows have a record type of 
"Household" and others have a record type of "Person". Variables that do not 
apply to a particular record type will be filled with `NA` in rows of that 
record type.

To read data in list format, use `read_ipums_micro_list()`. This function 
returns a list where each element contains all the records for a given 
record type:
...
## Reading Aggregate Data Extracts
...
`ipums_ddi` object, but many fields that do not apply to aggregate data
...
```{r}
read_nhgis_codebook(nhgis_ex1, raw = TRUE)[1:20]
```
...
file to load. `file_select` supports 
[tidyselect helpers](https://tidyselect.r-lib.org/reference/language.html) for
...
specifications. If the guessed specifications are incorrect, you can use
`read_csv()` arguments to adjust. This is most likely to occur for
...
Note that some readr options may conflict with ipumsr data loading
conventions. For instance, codebook metadata are attached by matching on
column name. Altering names on load will prevent the addition of
metadata for these columns (though the metadata can still be viewed in
the codebook itself).
...
read_nhgis(nhgis_fwf, file_select = matches("ts_nominal"))
```
...
It is possible to load fixed-width data manually using arguments passed
to `readr::read_fwf()` but this will not include handling of implicit
decimals, and you may end up with confusing or incorrect values. In
general, if you no longer have access to the .do file, it is best to
resubmit and/or re-download the extract (you can also consider
...
-   For microdata projects, spatial data are distributed in shapefiles
    on dedicated geography pages separate from the standard
...
Whether harmonized or not, use `read_ipums_sf()` to load spatial data
from any of these sources (ipumsr is phasing out support for objects
from the `sp` package. If you prefer to work with these objects, use
`sf::as_Spatial()` to convert from `sf` to `sp`).
...
`file_select` in this example because there is only one shapefile in this 
example extract).
