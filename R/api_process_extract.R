...
#'
#' @noRd
ipums_extract_request_url <- function(collection, path, queries = NULL) {
...
ipums_api_extracts_request <- function(verb,
  ...
  validate_api_extracts_request(response, url, call = caller_env())
...
# Helper to handle errors and warnings for requests to IPUMS API extract
# endpoints
validate_api_extracts_request <- function(res, url, call = caller_env()) {
  if (httr::http_status(res)$category != "Success") {
    ...
    if (status == 400) {
      ...
      rlang::abort(error_details, call = call)
    } else if (status == 404) {
      if (fostr_detect(url, "extracts/")) {
        ...
        rlang::abort(
          paste0(
            ...
            " does not exist for this collection."
          ),
          call = call
        )
      }

      rlang::abort("URL not found", call = call)
      ...
      rlang::abort(
        c(
          paste0(
            "Extract API request failed with status ", httr::status_code(res)
          ),
          paste0("URL: ", url),
  ...
  if (httr::http_type(res) != "application/json") {
    rlang::abort("Extract API did not return json", call = call)
  }

  invisible(NULL)
}

#' Helper function to retrieve paginated data via API for extract endpoints
#'
#' @param collection The IPUMS data collection for the extract.
#' @param path Extensions to add to the base url.
#' @param page_size An integer for the number of items to retrieve for each
#'   page. Default is 1500. This is also the maximum allowed page size.
#'
#' @return A list of items retrieved from the specified collection/path
#'   endpoints.
#'
#' @noRd
get_extract_pages <- function(collection,
                              page_size = 1500,
                              sleep = FALSE,
                              api_key = Sys.getenv("IPUMS_API_KEY")) {
  if (page_size > 1500) {
  ...
  extracts <- extract_list_from_json(response)

  # Extract objects will not contain nextPage link, so we need raw content too
  api_response <- jsonlite::fromJSON(response, simplifyVector = FALSE)

  while (!is.null(api_response$links$nextPage)) {
    # Fallback in case someone hits the rate limit somehow.
    # Sleeping for 1 sec should avoid limit.
    if (sleep) {
      Sys.sleep(1L)
    }

    # Get next page of extracts
    next_page_response <- ipums_api_extracts_request(
      "GET",
      collection = collection,
      url = api_response$links$nextPage,
    )

    this_page_extracts <- extract_list_from_json(next_page_response)
    extracts <- c(extracts, this_page_extracts)

    # Update api response, loop will continue if there are more pages to process
    api_response <- jsonlite::fromJSON(
      next_page_response,
